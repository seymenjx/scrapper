...
if parsed_url.scheme not in ("redis", "rediss"):
    logger.error(
        f"Invalid REDIS_URL scheme: {parsed_url.scheme}. Expected 'redis' or 'rediss'."
    )
    raise ValueError(
        f"Invalid REDIS_URL scheme: {parsed_url.scheme}. Expected 'redis' or 'rediss'."
    )
            ...
            for year, data_str in all_years.items():
                ...
                if data["status"] == "pending":
                    ...
                    pipeline.hset(
                        "scraping_progress",
                        year,
                        json.dumps({**data, "status": "in_progress"}),
                    )
        ...
        async with aioboto3.Session().client(
            ...
            try:
                while start_number <= end_number:
                    ...
                    logger.info(
                        f"Processing batch for year {year}: {start_number} to {batch_end}"
                    )

                    search_results = await search_records(
                        session, year, start_number, batch_end, current_page
                    )
                    ...
                    if not data:
                        logger.error(
                            f"No 'data' field in search results for year {year}"
                        )
                        break
                    ...
                    if not records:
                        logger.info(
                            f"No more results for year {year} in range {start_number} to {batch_end}"
                        )
                    ...
                    for record in records:
                        ...
                        if doc_id and esas_no and karar_no:
                            task = asyncio.create_task(
                                process_document(
                                    session, s3, year, doc_id, esas_no, karar_no
                                )
                            )
                            tasks.append(task)
                        else:
                            logger.warning(
                                f"Incomplete record data for year {year}: {record}"
                            )
